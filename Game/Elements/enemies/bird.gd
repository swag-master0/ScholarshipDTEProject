...
@export var attack_speed: float = 1200
...
func _physics_process(delta):
	...
	if ray.is_colliding():
		if ray.get_collider().is_in_group("player"):
			...
			velocity += ((ray.get_collider().global_position - global_position).normalized() * speed) * delta
	...
	if push.has_overlapping_bodies():
		for i in push.get_overlapping_bodies():
			if i != self and !i.is_in_group("player"):
				velocity -= (i.global_position - self.global_position) * delta
